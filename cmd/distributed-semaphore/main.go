...
func consistencyTest(ctx context.Context, rp ResourcePool, numWorkers int, duration int) bool {
...
func consistencyWorker(
	ctx context.Context, id int, rp ResourcePool, done <-chan struct{}, wg *sync.WaitGroup,
) {
	for {
		select {
		case <-done:
...
func validator(ctx context.Context, duration int, check func() (bool, error)) bool {
...
func progressTest(ctx context.Context, rp ResourcePool, numWorkers int, duration int) bool {
	var done = make(chan struct{})
	rwmut := &sync.RWMutex{}
	var wg sync.WaitGroup
	for i := 0; i < numWorkers; i++ {
