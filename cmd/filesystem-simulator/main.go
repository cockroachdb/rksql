...
import (
	...
	"github.com/cockroachdb/rksql/pkg/util/crdbutil"
	"github.com/cockroachdb/rksql/pkg/util/log"
	"github.com/cockroachdb/rksql/pkg/util/randutil"
	"github.com/cockroachdb/rksql/pkg/util/sqlutil"
...
func createFile(
	ctx context.Context, tx *sql.Tx, parentUUID string, name string, fileType fileType,
) (string, error) {
...
func execOneRow(
	ctx context.Context, tx *sql.Tx, s string, msgPrefix string, args ...interface{},
) (err error) {
	var res sql.Result
...
func getChildMap(ctx context.Context, tx *sql.Tx, uuid string) (map[string]string, error) {
...
func removeRandomFile(ctx context.Context, tx *sql.Tx) (*file, error) {
...
func printDirectoriesRecursive(ctx context.Context, tx *sql.Tx, uuid string, path string) error {
...
func printDirectories(ctx context.Context, rd *sqlapp.RobustDB, uuid string, path string) {
	executeTxOrDie(
		ctx,
		rd,
...
func selectFilesWhere(
	ctx context.Context, tx *sql.Tx, where string, args ...interface{},
) ([]file, error) {
	rows, err := tx.QueryContext(
		ctx,
		fmt.Sprintf(
...
func selectChildRelations(ctx context.Context, tx *sql.Tx, chrs *[]childRelation) error {
	rows, err := tx.QueryContext(
		ctx,
...
func selectStripes(ctx context.Context, tx *sql.Tx, ss *[]stripe) error {
	rows, err := tx.QueryContext(
		ctx,
...
func selectTxnTimestamp(ctx context.Context, tx *sql.Tx, txnTimestamp *string) error {
	const stmt = "SELECT cluster_logical_timestamp()::STRING"
	return tx.QueryRowContext(ctx, stmt).Scan(txnTimestamp)
}

func checkProgress(
	ctx context.Context, rd *sqlapp.RobustDB, opLog *opLog, durationSecs int, numWorkers int,
) {
...
func checkConsistency(ctx context.Context, db *sqlapp.RobustDB, id string) (bool, error) {
	var stripes []stripe
	var files []file
	var childRelations []childRelation
...
func createFileAndRecordOp(
	ctx context.Context, rd *sqlapp.RobustDB, id string, fileType fileType, opLog *opLog,
) {
...
func writeFileAndRecordOp(ctx context.Context, rd *sqlapp.RobustDB, id string, opLog *opLog) {
	var modifiedUUID string
...
func runRandomOp(ctx context.Context, rd *sqlapp.RobustDB, id string, log *opLog) {
...
func executeTxOrDie(ctx context.Context, rd *sqlapp.RobustDB, fn func(tx *sql.Tx) error) {
...
func runTest(
	ctx context.Context, rd *sqlapp.RobustDB, numWorkers int, numCheckers int, durationSecs int,
) {
	opLog := newOpLog()
