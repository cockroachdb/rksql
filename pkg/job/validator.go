...
func postCheckTimeout(
	jobPeriodScaleMillis int64, testDuration int64, numRoaches int64,
) time.Duration {
...
func awaitOneAdditionalSuccess(
	ctx context.Context, db *gorm.DB, jobSpecs []jobSpec, stop chan struct{},
) {
...
func assertJobsHaveArtifacts(ctx context.Context, db *gorm.DB, workerIndex int, numWorkers int) {
...
func assertArtifactsHaveJobs(ctx context.Context, db *gorm.DB, workerIndex int, numWorkers int) {
...
func scanInstancesWithJobID(
	tx *gorm.DB, jobID string, instanceID int64, limit int,
) ([]*ArchivedInstance, error) {
	var err error
	var instances []*ArchivedInstance
	err =
...
func scanArtifacts(
	tx *gorm.DB, workersIndex int, numWorkers int, artifactID string, limit int,
) ([]*Artifact, error) {
	var err error
	var artifacts []*Artifact
	err =
...
func hasValidJobInstance(
	ctx context.Context, tx *gorm.DB, artifact *Artifact,
